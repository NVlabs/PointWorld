...
- Local PTv3 license file: ptv3/LICENSE
"""
...
def prepare_flash_attn_dtype(tensor: torch.Tensor) -> torch.Tensor:
    ...
    return tensor.to(target_dtype) if tensor.dtype != target_dtype else tensor
...
class SerializedAttention(PointModule):
    def __init__(
        ...
        if enable_rpe:
            raise ValueError("RPE is not supported with FlashAttention-only mode.")
    ...
    @torch.no_grad()
    def get_padding_and_inverse(self, point, prefix=""):
        pad_key = f"{prefix}pad"
        unpad_key = f"{prefix}unpad"
        cu_seqlens_key = f"{prefix}cu_seqlens_key"
        if (
        ...
        pad, unpad, cu_seqlens = point[pad_key], point[unpad_key], point[cu_seqlens_key]
...
class Block(PointModule):
    def __init__(
        ...
        AttentionClass = self._attention_cls or SerializedAttention
    ...
    def forward(self, point: Point):
        ...
        if self.checkpoint_attn:
            point = torch.utils.checkpoint.checkpoint(self.attn, point, use_reentrant=False)
        else:
...
class GridPooling(PointModule):
    ...
    def forward(self, point: Point, skip_postprocess=False):
...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        enc_checkpoint_attn=None,
        dec_depths=(2, 2, 2, 2),
        dec_channels=(64, 64, 128, 256),
        dec_num_head=(4, 4, 8, 16),
        dec_patch_size=(48, 48, 48, 48),
        dec_checkpoint_attn=None,
        mlp_ratio=4,
        qkv_bias=True,
        qk_scale=None,
        attn_drop=0.0,
        ...
        if enc_checkpoint_attn is None:
            enc_checkpoint_attn = [False] * len(enc_depths)
        if dec_checkpoint_attn is None:
            dec_checkpoint_attn = [False] * len(dec_depths)
            ...
            for i in range(enc_depths[s]):
                enc.add(
                    Block(
                        ...
                        enable_rpe=enable_rpe,
                        checkpoint_attn=enc_checkpoint_attn[s],
                ...
                for i in range(dec_depths[s]):
                    dec.add(
                        Block(
                            ...
                            cpe_indice_key=f"stage{s}",
                            enable_rpe=enable_rpe,
                            checkpoint_attn=dec_checkpoint_attn[s],
                            attention_cls=self.attention_cls,
                            attention_kwargs=self.attention_kwargs,
                        ),
                        name=f"block{i}",
