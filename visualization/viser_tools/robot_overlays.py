...
def build_robot_flows(
    ...
    *,
    ...
    - Else, use generic URDF FK with required (joint_names, joint_positions_full).
      Missing kinematic inputs are treated as errors (fail-fast).
    ...
    if use_panda:
        ...
        if joint_names is None or joint_positions_full is None:
            raise ValueError(
                "Missing robot kinematics for generic URDF overlays: "
                "expected both 'joint_names' and 'joint_positions_full'."
            )
