...
Additional routines are adapted from deoxys_control where noted in-file:
- Upstream: https://github.com/UT-Austin-RPL/deoxys_control
...
try:
    ...
    njit = lambda cache, fastmath: lambda x: x
import numpy as np
...
try:
    ...
    print("Numba not found, using fallback implementation")
    njit = lambda cache, fastmath: lambda x: x

# ----------------------------------------------------------------------
#  Tiny, JIT-able rotation helpers
# ----------------------------------------------------------------------
...
def _mat2quat_single(R):
    ...
    Uses Shoemake’s 1992 branch-on-trace method.
    """
    t = R[0, 0] + R[1, 1] + R[2, 2]
    if t > 0.0:
        s = 0.5 / np.sqrt(t + 1.0)
...
def convert_pose_mat2quat(poses_mat: np.ndarray) -> np.ndarray:
    """
    (4,4)  or (N,4,4) → (7,) or (N,7)  [x y z qx qy qz qw]
    """
...
def convert_pose_quat2mat(poses_quat: np.ndarray) -> np.ndarray:
    """
    ...
    out[:, 3, :] = np.array((0., 0., 0., 1.), dtype=poses_quat.dtype)  # fix bottom row
...
def quat_distance_batched(quaternions1, quaternions0):
    """
    Returns distance between two lists of quaternions.

    Args:
        quaternions1 (np.array): (N, 4) quaternions
        quaternions0 (np.array): (N, 4) quaternions
    
    Returns:
        np.array: (N, 4) quaternion distances
    """
...
def average_poses(poses_4x4):
    """
    Roughly average a set of 4x4 transforms by:
        1) average translations directly
        2) sum rotation matrices and do SVD => orthonormal
    Return a single 4x4.
    """
    if len(poses_4x4) == 1:
        return poses_4x4[0]
    ...
    for p in poses_4x4:
        R, t = rotation_translation_from_4x4(p)
        R_sum += R
        t_sum += t
    R_sum /= len(poses_4x4)
    t_sum /= len(poses_4x4)
    # Orthonormalize R_sum
    U, S, Vt = np.linalg.svd(R_sum)
    R_ortho = U @ Vt
    if np.linalg.det(R_ortho) < 0:  # fix possible reflection
        R_ortho[:, -1] *= -1
    out = np.eye(4)
    out[:3, :3] = R_ortho
    out[:3, 3] = t_sum
...
def rotation_error_deg(R1, R2):
    ...
    Uses the formula: θ = arccos((trace(R1^T * R2) - 1) / 2)
    """
    # Ensure we're dealing with proper rotation matrices
    ...
    # Compute the error
    dot_product = np.trace(R1.T @ R2)
    # Clip the value to handle numerical errors
    dot_product = np.clip(dot_product, -3.0, 3.0)
    angle_rad = np.arccos((dot_product - 1) / 2)
